...
import { deleteLessonApi, lessonListApi, publishLessonApi } from '@service/academic';
...
const ClassManagement: React.FC = () => {
        ...
        return (
          <Space size='small' wrap>
            <Button
              type='link'
              style={{ padding: '0' }}
              onClick={() => navigate('/academic/class-manage/add-class', { state: value } )}
            >
              编辑
            </Button>
  ...
  const publishLesson = async (id?) => {
    ...
    const { data, status } = await publishLessonApi(params);
    if (status !== '00000') { return; }
  ...
  const deleteLesson = async (id) => {
    Modal.confirm({
      ...
      content: '确定要删除吗',
      onOk: async () => {
        const { data, status } = await deleteLessonApi({
          id,
          modifier: userName
        });
        if (status !== '00000') { return; }
        ...
        >
          <Row wrap gutter={24}>
            ...
            <Col lg={{ span: 12 }} xl={{ span: 8 }} className="text-right">
              <Button type="primary" onClick={() => { navigate('/academic/class-manage/add-class') }}>新建</Button>
            </Col>
          </Row>
          {expand && <>
            <Row wrap gutter={24}>
              <Col lg={{ span: 12 }} xl={{ span: 4 }}>
                <Form.Item name="id" label="课时ID">
                  <Input placeholder="请输入" />
                </Form.Item>
              </Col>
              <Col lg={{ span: 12 }} xl={{ span: 4 }}>
