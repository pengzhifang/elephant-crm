    ...
    >
      ...
      >
        ...
        <Form.Item
          ...
          label="句子"
          rules={[
            { required: true, whitespace: true, message: '请输入句子!' }
          ]}
        >
            ...
            {languages.map((item, index) => {
              return (
                <div className="flex mb-3" key={item.id}>
                  <div className="w-[50px] text-right mr-2">{item.code == 'en-US' && <span className="required"></span>}{item.name}</div>
        ...
        >
          <div className="flex flex-wrap items-center w-[80%] mt-[5px] space-y-[5px]">
            {audioList.map((item, i) => {
              return (
                <div className="mr-6 relative flex items-center" key={item.code}>
                  <div className="mb-1 w-[50px] text-right mr-3">{item.code == 'zhinan' && <span className="required"></span>}{item.name}</div>
                  {
                    item.url ?
                      <div><Button type="primary" className="w-[65px]" onClick={() => { playAudio(item.url) }}>播放</Button><CloseCircleFilled className="text-[red] text-xs absolute right-[-16px] top-[8px]" onClick={() => { deleteAudio(item.code) }} /></div>
                      :
                      <Upload
                        className="w-[65px]"
                        name='audio'
                      ...
                      >
                        {
                          item.loading ?
                            <span className="w-[65px] h-[32px] flex justify-center items-center rounded-[6px] border border-solid border-[#d9d9d9]"><LoadingOutlined /></span>
                            :
                            <Button className="w-[65px]">上传</Button>
                        }
                      </Upload>
                  }
                </div>
